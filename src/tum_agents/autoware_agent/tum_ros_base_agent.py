...
class BridgeHelper(object):
    ...
    def carla2ros_pose(cls, x, y, z, roll, pitch, yaw, to_quat=False):
        ...
        # Fixed bug in orientation of angles according to
        # https://github.com/carla-simulator/ros-bridge/issues/637
...
class TUMROSBaseAgent(AutonomousAgent):
    ...
    def __init__(self, ros_version, carla_host, carla_port, debug=False):
        super(TUMROSBaseAgent, self).__init__(carla_host, carla_port, debug)

        # Manual matching of the map of the CARLA Server and the CARLA-ROS-Bridge
        # to avoid crashes
